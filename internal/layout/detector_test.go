...
func TestDetectLayoutHorizontalCuts(t *testing.T) {
	// Create page with two blocks separated by a gap > MinGapThreshold
	page := &model.Page{
		Number: 1,
		Width:  500,
		Height: 700,
		Flows: []model.Flow{
			{
				...
				Blocks: []model.Block{
					{XMin: 50, YMin: 50, XMax: 450, YMax: 100},   // First band
					{XMin: 50, YMin: 150, XMax: 450, YMax: 200},  // Second band (gap of 50pt)
				},
			},
		},
	}
	...
	// Should detect 1 horizontal cut between the two bands
	if len(layout.HorizontalCuts) != 1 {
		t.Fatalf("Expected 1 horizontal cut, got %d", len(layout.HorizontalCuts))
	}

	cut := layout.HorizontalCuts[0]
	if cut.Y < 100 || cut.Y > 150 {
		t.Errorf("Expected cut Y between 100 and 150, got %f", cut.Y)
	}
	if cut.XMin != 0 || cut.XMax != 500 {
		t.Errorf("Expected cut to span page width [0, 500], got [%f, %f]", cut.XMin, cut.XMax)
	}
}
